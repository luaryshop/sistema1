...
export const inventoryRouter = router({
  ...
  availableToSell: protectedProcedure.input(z.object({ productId: z.number().int().positive(), variantId: z.number().int().positive().optional() })).query(({ ctx, input }) => InventoryService.availableToSell(ctx.user.id, input.productId, input.variantId)),
